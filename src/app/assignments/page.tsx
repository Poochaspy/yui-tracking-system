    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage and track tasks assigned to personnel.</p>
        </div>
              ...
              {assignments.length === 0 ? (
                ...
                assignments.map((assignment: any) => (
                  <tr key={assignment._id} className="border-b border-slate-100 hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-slate-500">{assignment.assignedTo?.name || 'Unknown'}</td>
                    <td className="p-4 text-slate-500">{assignment.assignedModel}</td>
                    <td className="p-4">
                      <TaskStatusButton currentStatus={assignment.status} assignmentId={assignment._id} />
                    </td>
                    <td className="p-4">
                      <CreateAssignmentModal personnel={personnel} assignment={assignment} />
                    </td>
                  </tr>
                ))
              )}
