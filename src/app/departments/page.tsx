    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage hospital departments and their capacity.</p>
        </div>
        ...
        {departments.length === 0 ? (
          ...
          departments.map((dept: any) => (
            <div key={dept._id} className="glass-card rounded-2xl p-6 hover:border-indigo-500/30 transition-colors">
              ...
              <h3 className="text-xl font-semibold mb-2 text-slate-800">{dept.name}</h3>
              <p className="text-sm text-slate-500 line-clamp-2">
                {dept.description || 'No description provided.'}
              </p>
              <CreateDepartmentModal department={dept} />
            </div>
          ))
        )}
      </div>
